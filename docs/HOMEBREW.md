...
Pass the already-published package version, for example `0.7.2`.
...
python scripts/render_homebrew_formula.py --package-version 0.7.2 --output /tmp/homebrew-tap/Formula/voice-dashboard.rb
...
```bash
python scripts/render_homebrew_formula.py --package-version 0.7.2
```
